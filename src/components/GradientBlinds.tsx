...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  const mouseDirRef = useRef<[number, number]>([0, 0]);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Use mouse direction to shift the gradient or shine
    float dirStrength = length(uMouseDir);
    float dirAngle = atan(uMouseDir.y, uMouseDir.x);
    // Example: shift t by direction for a reactive effect
    float tShift = t + 0.1 * dirStrength * cos(dirAngle - uAngle);
    vec3 base = getGradientColor(tShift);
...
`;
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const prev = lastMouse;
      lastMouse = [x, y];
      const dx = x - prev[0];
      const dy = y - prev[1];
      mouseDirRef.current = [dx, dy];
      uniforms.uMouseDir.value = [dx, dy];
    };
    ...
    const loop = (t: number) => {
      ...
      // Decay mouse direction for smoothness
      uniforms.uMouseDir.value[0] *= 0.92;
      uniforms.uMouseDir.value[1] *= 0.92;
